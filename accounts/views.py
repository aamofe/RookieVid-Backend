...
def send_mail_vcode(request):
    ...
    if mail_code_time and now_time < mail_code_time + 60:  # 1分钟内不能重复发送邮件
        ...
        if (send_status == 1):
            resp = {'status': 1000,'msg': '验证码已发送，请查阅'}
            
        else:
            return JsonResponse({'from': EMAIL_FROM, 'to': to_email, 'status': 1006, 'msg': "验证码发送失败，请检查邮箱地址"})
...
def register(request):
    if request.method == 'POST':  # 判断请求方式是否为 POST（要求POST方式）
        ...
        if session_email and session_email == email:
            ...
            if session_code_time and now_time <= session_code_time + 300:
                ...
                if session_code and session_code == vcode:
                    ...
                    return JsonResponse({'status': 1007, 'msg': '验证码错误'})
            else:
                return JsonResponse({'status': 1008, 'msg': '验证码失效，请重新获取'})
        else:
            return JsonResponse({'status': 1009, 'msg': '该账户没有获取验证码'})
        # 验证码正确，进行注册
        ...
        if User.objects.filter(email=email).exists():
            return JsonResponse({'status': 1010, 'msg': "账号已存在，请勿重复注册"})
        # 用户名长度为1-20位
        if re.match('.{1,20}', str(username)) is None:
            return JsonResponse({'status': 1001, 'msg': "用户名不合法"})
        # 密码长度为8-16位，且同时包含数字和字母
        if re.match('(?!^[0-9]+$)(?!^[a-zA-Z]+$)[0-9A-Za-z]{8,16}', str(password_1)) is None:
            return JsonResponse({'username':username, 'password': password_1, 'status': 1002, 'msg': "密码格式错误"})
        if password_1 != password_2:
            return JsonResponse({'status': 1003, 'msg': "两次输入的密码不同"})
        else:
            # 为用户分配不重复的uid
            ...
            return JsonResponse({'uid': uid, 'status': 0, 'msg': "注册成功"})
    else:
...
def login(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(uid=uid).exists():
            ...
            return JsonResponse({'status': 1001, 'msg': "请先注册"})
        if user.password == password:  # 判断请求的密码是否与数据库存储的密码相同
            ...
            return JsonResponse({'status': 1002, 'msg': "密码错误"})
    else:
        return JsonResponse({'error': 1, 'msg': "请求方式错误"})
...
def logout(request):
    ...
    return JsonResponse({'status': 0, 'msg': "注销成功"})
